function handles = distributionPlot_OnlyPositive(varargin)
% IMPORTANT
%
%
%      This is a very simple and imperfect modification of the wonderful code 'distributionPlot',
%      originally developed by Jonas Dorn (https://fr.mathworks.com/matlabcentral/fileexchange/23661-violin-plots-for-plotting-multiple-distributions-distributionplot-m)
%
%      In this modification, the ksdensity function is set to the correction of density estimation that
%      prevent to give negative results. otherwise it works exactly as the original function, and the call 
%      is identical.
%            
%
%      ---------- R.G. Bettinardi, 11th December 2018 ----------------
% 
%
% 
...
if ~iscell(data)
    ...
    % get nData
    addeps = @(x) x+eps;
    data = cellfun(addeps,data,'UniformOutput',false);
    data = data(:);
    nData = length(data);
        ...
        switch floor(opt.histOpt)
            ...
            case 1
                ...
                if opt.histOpt == 1.1
                    ...
                    [xHist,yHist] = ksdensity(currentData,x,'kernel','normal','width',dx/(1.5*opt.divFactor),'Support','positive','BoundaryCorrection','reflection');
                    end
                else
                    
                    % x,y are switched relative to normal histogram
                    [xHist,yHist,u] = ksdensity(currentData,'kernel','normal','Support','positive','BoundaryCorrection','reflection');
                    % take smaller kernel to avoid over-smoothing
                    if opt.divFactor ~= 1
                        [xHist,yHist] = ksdensity(currentData,'kernel','normal','width',u/opt.divFactor,'Support','positive','BoundaryCorrection','reflection');
                    end
                end
