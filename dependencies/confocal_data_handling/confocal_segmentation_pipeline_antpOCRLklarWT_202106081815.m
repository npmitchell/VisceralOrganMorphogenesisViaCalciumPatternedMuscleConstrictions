...
% WT
dataDir = '~/Desktop/gut/optogenetics_confocal/202106081815_48YG4kCAAXmCh_0p75um_1p5x40x_3t6pc3t6pc_lav3_615ns_5mpf/' ;
...
tp0 = 7 ;  % which timepoint (index) is the onset of folding?
embryoView = 'LD' ; % 'RD'/RV/LD/LV --> 
...
if data_is_split
    ...
    for tidx = 1:length(fns0)
        ...
        if ~exist(fn2, 'file') || overwrite
            if exist(fn2, 'file')
                go_on = input('Are you sure you want to overwrite?', 's') ;
                if contains(lower(go_on), 'n')
                    error('stopping!')
                end
            end
            ...
            writeTiff5D(permute(im, [1, 2, 4, 3]), fn2, bitDepth)
