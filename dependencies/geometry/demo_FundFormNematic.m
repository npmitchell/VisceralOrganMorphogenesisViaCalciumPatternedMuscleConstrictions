...
outdir = '/mnt/data/analysis/demo_HopfDifferential/' ;
if ~exist(outdir, 'dir')
    mkdir(outdir)
end
...
for ii = id2do
    ...
    opts.axisOff = true ;
    opts.labels = {'', 'Hopf differential, $\theta=-\tan^{-1}(\Im Q/\Re Q)$', ''} ;
    opts.views = {[45, 45 ], [0, 90 ], [45,45]};
    opts.phasebarPosition = [0.33, 0.20, 0.1, 0.135] ;
    opts.xyzlims = 0.5 * [-1, 1;-1,1;-1,1] ;
    opts.cbarlabels = {['Hopf differential, $|Q|$ [L$^{-2}$]'], ...
                       ['Hopf differential, $|Q|$ [L$^{-2}$]'], ...
