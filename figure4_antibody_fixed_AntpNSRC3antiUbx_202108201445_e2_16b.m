...
% temporary path def
cd /mnt/data/antibodies_lightsheet/AntpNSRC3_48YGal4CAAXmChUbxFP3p38_1t20_ms488_NucRed647/202108201445_e2_16b_1p2um_0p2ms0p5ms_3mW_15mW_4v/data/
dataDir = pwd;
...
if overwrite_masterSettings || ~exist('./masterSettings.mat', 'file')
    ...
    scale = [0.3 0.17] ;      % scale for conversion to 16 bit
...
if ~exist(fileNameOut, 'file') || overwrite
    ...
    disp('Combined TIFF already exists on file.')
end
...
% FIRST RAN
command1 = ['python /mnt/data/code/morphsnakes_wrapper/morphsnakes_wrapper/run_morphsnakes.py  ' ...
     '-i fixed_sample_combined_Probabilities.h5 -o  ' ...
     fullfile(dataDir, 'msls_output'), ...
     ' -prenu -6 -presmooth 1 -ofn_ply msls_ms_000000.ply -ofn_ls msls_000000.h5   ' ...
     '-l1 1 -l2 1 -nu 0.5 -postnu 3 -channel 1 -smooth 4  -postsmooth 3  ' ...
     '-exit 0.000001000 -channel 1 -dtype h5 -permute czyx  ' ...
     '-ss 4 -include_boundary_faces -rad0 10   ' ...
     '-init_ls ' dataDir '/msls_output/msls_initguess.h5  '...
     '-n 50 -save'] 

% THEN RAN
command2 = ['python /mnt/data/code/morphsnakes_wrapper/morphsnakes_wrapper/run_morphsnakes.py ', ...
    '-i fixed_sample_combined_Probabilities.h5 -o ', ...
    fullFile(dataDir, 'msls_output'), ' -prenu -5 -presmooth 1 ', ...
    '-ofn_ply mesh_ms_000001.ply -ofn_ls msls_000001.h5 -l1 1 -l2 1 ', ...
    '-nu 0.5 -postnu 3 -smooth 4 -postsmooth 3 -exit 0.000010000 ', ...
    '-channel 1 -dtype h5 -permute czyx -ss 4 -include_boundary_faces ', ...
    '-init_ls ', fullfile(dataDir, 'msls_output/msls_initguess.h5'), ...
    ' -save -rad0 10 -n 14'] 
...
if ~exist(preSmoothMeshfn, 'file')
    ...
    tmp = mesh ;
    trisurf(triangulation(tmp.f, tmp.v), tmp.vn(:, 3), 'edgecolor', 'none'); 
    axis equal
    ...
    tmp.v = tmp.v + 10 * tmp.vn ;  % + (amesh.v(:, 1)/ 100) .* tmp.vn;
    trisurf(triangulation(tmp.f, tmp.v), tmp.vn(:, 3), 'edgecolor', 'none')
    axis equal
...
overwrite_APDVCOMs = true;
overwrite_APDVMeshAlignment = true ;
overwrite_alignAPDVOpts = true;
...
if ~exist(metafn, 'file') || overwrite_TextureMeshOpts
    ...
    metadat.normal_shift = 0; %QS.normalShift ;             % normal push, in pixels, along normals defined in data XYZ space
    ...
    Options.numLayers = [12, 5];  % at layerSpacing=2, numLayers=2 marches ~0.5 um 
...
amprettyOpts.plot_perspective = false ;
